...
function Explanation({ question, selected }: { question: Question; selected: number | null }) {
  ...
  const detail = isCorrect ? question.whyCorrect || question.explanation : question.optionExplanations?.[String(selected)] || question.explanation;
  return (
    <div className={`explanation-block ${isCorrect ? "is-correct" : "is-wrong"}`}>
      <strong>{isCorrect ? "Đáp án đúng" : "Bắp nhầm ở đâu?"}</strong>
      <p>{detail || "Chưa có giải thích chi tiết cho câu này."}</p>
      {!isCorrect && <p className="explanation-correct">Đáp án đúng: {String.fromCharCode(65 + question.correct)}. {question.options[question.correct]}</p>}
      {question.memoryTip && <p className="explanation-tip"><strong>Nhớ nhanh:</strong> {question.memoryTip}</p>}
      {question.signExplanations && <ul className="sign-explanation-list">{question.signExplanations.map((item) => <li key={item.label}><strong>{item.label}:</strong> {item.meaning}</li>)}</ul>}
    </div>
  );
}
...
function HomeView({ progress, currentSession, sound, onSound, onStart, onWrong, onResume, onDiscard, onSettings, onChapter }: {
