...
function PresetPicker({ presets, progress, onBack, onStart }: { presets: PresetExam[]; progress: ProgressData; onBack: () => void; onStart: (preset: PresetExam) => void }) {
  const getPresetStatus = (presetId: number): { status: "passed" | "failed" | "none"; score?: number; total?: number; reason?: string } => {
    ...
    return { status: "failed", score: latest.score, total: latest.total, reason: lietWrong ? "Dính câu điểm liệt" : `Sai ${wrongCount} câu` };
  };
  return <main className="container page-section"><div className="preset-header"><button className="icon-button" onClick={onBack} aria-label="Quay lại"><ArrowLeft size={19} /></button><div><p className="eyebrow">Học trọn bộ câu hỏi</p><h1 className="page-title">20 bộ ôn cố định — phủ đủ 600 câu</h1><p className="page-subtitle">Mỗi câu trong bộ 600 xuất hiện một lần để Bắp có thể học hết ngân hàng câu hỏi.</p></div></div><div className="preset-grid">{presets.map((preset) => { const ps = getPresetStatus(preset.id); return <article className={`preset-card ${ps.status === "passed" ? "preset-passed" : ps.status === "failed" ? "preset-failed" : ""}`} key={preset.id} onClick={() => onStart(preset)} role="button" tabIndex={0}><div className="preset-topline"><span className="preset-number">{String(preset.id).padStart(2, "0")}</span><span className="soft-pill">30 câu</span>{ps.status === "passed" && <span className="preset-status-badge preset-badge-passed"><CheckCircle2 size={14} /> Đậu {ps.score}/{ps.total}</span>}{ps.status === "failed" && <span className="preset-status-badge preset-badge-failed"><X size={14} /> Rớt — {ps.reason}</span>}</div><h2 className="preset-title">{preset.title}</h2><p className="preset-detail">Bộ ôn cố định {preset.questions.length} câu từ ngân hàng 600 câu.</p><button className="secondary-button small-button preset-start-btn" onClick={(event) => { event.stopPropagation(); onStart(preset); }}>{ps.status !== "none" ? "Làm lại" : "Vào học"} <ChevronRight size={15} /></button></article>; })}</div></main>;
}
...
function ExamView({ session, index, answers, seconds, ready, sound, locked, onReady, onAnswer, onNext, onPrevious, onJump, onExit, onFinish }: {
  session: Session;
  index: number;
  answers: AnswerMap;
  seconds: number | null;
...
export function ExamApp() {
  ...
  const finish = () => {
    ...
    let score = 0;
    let lietWrong = false;
    const rows = session.questions.map((question) => { const selected = latestAnswers[question.id] ?? null; const isCorrect = selected === question.correct; if (isCorrect) score += 1; if (question.isLiet && !isCorrect) lietWrong = true; return { question, selected, isCorrect }; });
  ...
  const start = (mode: PracticeMode, customQuestions?: Question[], title?: string, presetId?: number) => {
    ...
    setSession(nextSession); setIndex(0); setAnswers({}); answersRef.current = {}; setSeconds(null); setReady(false); setResult(null); setView("exam"); commit({ ...progress, currentSession: { id: nextSession.id, mode, title: titleValue, questionIds: questions.map((question) => question.id), currentIndex: 0, answers: {}, startedAt: new Date().toISOString(), timer, remainingSeconds: null, timeLimitSeconds: limit, presetId } }); playSound("tap", sound); window.scrollTo({ top: 0, behavior: "smooth" });
  };
  const resume = () => {
